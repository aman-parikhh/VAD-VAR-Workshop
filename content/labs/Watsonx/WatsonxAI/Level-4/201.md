...
- You will also need to download the lab files from [this GitHub folder](https://github.com/CloudPak-Outcomes/Outcomes-Projects/raw/main/L4assets/watsonx.ai-Assets.zip)
...
3. Click on the **New Project (1)** button. Add your initials to the project name **(3)**. For example,  **LLM-workshop-YI**. Select Storage(if required), then, click on the **Create (4)** button.
...
5. Select the displayed **Machine Learning(watsonx.ai)** service and click **Associate**.
...
7. Click on the **Chat and build prompts with foundation models** tile.
    ...
    It may be possible to achieve similar results with other models or with smaller versions of _llama_ by using few-shot prompting or fine-tuning, that’s why it’s important to experiment with multiple models and understand prompt/turning techniques.


    > Note: Instructions in this lab are written for the flan and llama models, which are available in all IBM Cloud data centers where watsonx.ai is hosted and in the on-premises version of watsonx.ai. We encourage you to try other models (for example, granite and mpt-7b), if
    they’re available in your workshop environment.

8. After the **Prompt Lab** UI opens, switch to the **Freeform** tab. Select the **_flan-ul2-20b_** model. 
...
9.  Type in the question: `What is the capital of the United States?` and click **Generate**. The generated answer is highlighted in blue.
    ...
    Next, switch the model to _granite_ , and ask the question again. This time we get the correct answer.
    ![main](../images/201/L4-genai-14.png)


Next, switch the model to _mt0-xxl-13b_(if available) and ask the question again - we get another incorrect answer.
...
14. Finally, try the _llama-2-70b-chat_ model. For “creative output” use cases, _llama_ usually produces the best output.
    ...
    Your response might also differ from what is shown in this guide as the Decoding option is set to Sampling.
...
15. Click on the Sample Prompts icon and scroll down to the Marketing email generation example.
   ![](../images/201/add-1.png) 
   
You can run this example from either the Structured or Freeform view. If you use the Structured view, the input for generation is shown under Details.
   ![](../images/201/add-2.png) 


If you wish, change the input for the generated email and test the prompt. Select the TXT tab (next to the Model parameter tab in the upper quadrant of the interface) to see the full prompt. Copy the prompt and paste it in the Freeform view.
   ![](../images/201/add-3.png) 
...
19. In the **Prompt Lab** , create a new prompt. Switch to **Freeform** , and paste the following prompt(this prompt is available in the **Paragraph_few_shot.txt** file in the /Prompts folder):
...
21. Notice the token count that’s shown on the bottom of the model output.
    ...
    The token constraint limitation can be solved with several approaches. If we need to provide more examples to the model, we can use an approach called Multitask Prompt Tuning (MPT) or fine tuning. We are not covering these advanced approaches in this introductory lab.

    <br />
    Up to this point we reviewed question & answer and generation examples. We started with these examples because for most users they are the “first experience” with generative AI.
    
    <br />
    Many people are familiar with ChatGPT , a popular personal assistant application developed by OpenAI. Sometimes the terms generative AI and even LLM are used interchangeably with ChatGPT , but ChatGPT is more than an LLM, it’s a complex application that uses LLMs.

    <br />
    LLMs are building blocks or components of an application, and by themselves they can rarely be used by a business user. ChatGPT is a tool that focuses on personal productivity tasks for many types of users. Companies that want to build AI-driven applications need an
...
In this lab we will be using the Marketing email generation Sample prompt.
2. Generate a response using this prompt. 
- You can use either the flan or the granite model
...
- Make sure to set the min and max tokens to 50 and 300
    ...
    After testing the prompt, click **Generate** and then click on the **View code** icon.
    ...
    The **project id** can be looked up in the **Project -> Manage view** tab of the watxonx.ai project.

    ![id](../images/201/project_id.png)
...
4. The browser tab pointing to the **LLM-Workshop-YI** project should still be open in your browser. Select the tab to switch back to the **watsonx** platform.
5. Select the **Assets** tab and click **New asset** -> **Work with data and models Python or R notebooks**.
...
6. Find the following Python scripts in the downloaded git repo [applications](https://github.com/ibm-build-lab/VAD-VAR-Workshop/tree/main/content/labs/Watsonx/WatsonxAI/files/201/applications) folder:
...
7. In your Python IDE install the ibm-watson-machine-learning library. We recommend that you use Python 3.10 environment.
...
8. Run the script _demo_wml_api.py_. The output will be shown in the Python terminal.
