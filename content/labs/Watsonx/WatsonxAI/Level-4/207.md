...
In this lab, you will be introduced to the watsonx.ai synthetic data generator to generate synthetic data that can be used with traditional AI or foundation models for testing/tuning. This lab details two ways to generate synthetic data:
...
22. Click **Save**.
    ![save-1](../images/207/image-020.png)
...
32. You are returned to the **Generate synthetic tabular data flow** page. You now see all 3 columns you defined have been added.

33. Click **Next** to generate the data.

![generate](../images/207/32.png)

34. On the next panel, you can specify the output **File name**. Change it to **Name_Age_Salary**.

35. Click on the **File Type** drop-down and select **Excel**.

36. Click **Next**.

![export](../images/207/34.png)

37. On the next page, review the information. If it is correct, click **Save Flow**.

![save_and_run](../images/207/save-now.png)

38. On the next screen, click on Run flow.
    
![run_flow](../images/207/run-flow.png)
...
![](<../images/207/Screenshot 2025-05-12 at 4.56.21 PM.png>)
...
One of the key points in generating synthetic data is how closely the data resembles real-life data. It is important to keep in mind that to the AI model, there is no inherent meaning in **Last_Name**, **Age**, and **Salary**. These are simply tokens used. This means that it can sometimes
...
24.  You realize that setting the value for Correlation to 1 in Step 11 is the problem. To get a more realistic set of data, repeat Steps 1-12. This time setting the value of Correlation to 0.8 in Step 11. Run through Steps 12-23 to re-generate and visualize the data again.
...
Repeat Steps 3 - 9 from the `Synthetic data generation` section above.
...
4. This time, select the **Leverage your existing data** use case on the **Select use case** tab.
...
Keep differential privacy at default. 

14. Click **Next**.

![fit_criteria](../images/207/image-055.png)

15. The next step is for data evaluation. We will explore both options. For now, let it be on default which is not enabling evaluate metric. 

16.  Click Next.
    
![](../images/207/new-207.png)

17. For **File Name**: Enter **mimic-output**

18. For **File type**, keep the default value of **CSV**
...
20.  Focus is now on the **Review** task. Click the **Edit** icon to make any updates.
...
21.   After verification click on Save flow. It could take a few seconds to save this flow.

![save_and_run](../images/207/image-058.png)

22. Click on Run Flow.

![run_flow](../images/207/22.png)

23.  The pipeline starts running and will take some seconds to run (it can vary depending on the availability of resources – anywhere from several seconds to tens of seconds, so be patient if it takes a little longer). You will see a quick summary as follows:

![pipeline](../images/207/pipeline.png)

24. The next page shows a graphical representation of the tasks. There are 4 main areas:
...
25.  **Run selection** – you can choose to run Generate on its own (to do so, click the **Generate** tile before clicking **Run** selection) before running the **Export** pipeline; or you can choose to run both steps in one pass.

![Run Selection](../images/207/run-selection.png)

26.  Go to the breadcrumb and click your project.

![project](../images/207/image-061.png)

27. You will see the following (you may have other assets):
...
![assets](../images/207/27.png)

28. The output should look something like this:

![output](../images/207/image-063.png)

29.  Now let's compare the distribution of the original data set and the generated dataset. For this, we will use the profile tab. Click on the generated dataset (mimic-output.csv).

![cmp](../images/207/29.png)

30. Click on the Profile tab.

![prof](../images/207/30.png)

31. Click on Profile data. This could take a while. 
 
![prof-2](../images/207/31.png)


32. Repeat the same for the original dataset (titanic.csv). The frequency distribution for the feature **Gender** looks similar in both the datasets.

![gender](../images/207/cmpre.png)

Feel free to experiment further as well.

### Generating data with a seed file and evaluation metrics

In this section, you generate synthetic data using the same seed file and also enable evaluation metrics.  
Repeat steps **1 to 22** from the above section with the following updates:

- For **Name:** `Sales data generation with seed and evaluation metric`  
- For **Description:** `Generate a large synthetic sales data set using a seed file and enabling evaluation metrics`

---

1. **Enable evaluate metrics** and select the following:
   - **Fidelity score**  
   - **Data distinguishability** 
   - **Leakage score**  
   - **Proximity score**  
   Keep the **Assessment level** at `Simple assessment`.

2. Click **Next**.

![](../images/207/evaluate-1.png)

3. **Name the output file**: `mimic-output-eval.csv`  
   The **File Type** is `CSV`.

4. Click **Next**.
![](../images/207/evaluate-2.png)


5. On the next screen, click on **Save flow**, followed by **Run flow**.
![](../images/207/r-f.png)


6. This time there is an additional **Evaluate** task, along with the previous steps: **Import**, **Mimic**, **Generate**, and **Export**.
![](../images/207/evaluate-4.png)


7. From the **Output bar**, click on **Evaluate** to view the **Evaluation Results**.
![](../images/207/evaluate-5.png)


#### Evaluation results:

- **Fidelity score**  
  This metric combines multiple measures to assess how closely the synthetic data resembles the real data. It evaluates both the **distribution of individual columns** and the **correlations between pairs of columns**. A **fidelity score of 72.35%** indicates that the synthetic data is **moderately similar** to the real data.

- **Data distinguishability**  
  This metric gauges the ability of a binary classifier to differentiate between real and synthetic data. Higher difficulty in training such a classifier indicates that the synthetic data better mirrors the statistical properties of the real data. A **data distinguishability score of 15.9%** signifies that it is quite difficult for a classifier to differentiate between the real and synthetic data. This low score means the synthetic data **closely mimics** the statistical properties of the real data, making it **hard to distinguish**.

- **Leakage score**  
  This metric indicates the fraction of rows in the synthetic data that are **exact duplicates** of rows in the real data. A **leakage score of 100%** means that **every row** in the synthetic data is **identical to some row** in the real data.

- **Proximity score**  
  This metric is based on the **distance between points** in the synthetic and real data. Smaller distances imply that it's easier to isolate some rows from the real data, thus **increasing privacy risk**. A **proximity score of 79.86%** suggests that the synthetic data points are **quite close** to the real ones.

---

You can follow the **rest of the steps as-is** from the above section to generate and compare the synthetic data with the original dataset.
...
- You can add evaluation metrics to evaluate the quality of synthetically generated data.
