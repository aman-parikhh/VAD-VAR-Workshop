...
4. Ensure that you are using the **flan-ul2-20b** model. Copy and paste the following text in the input field:
   ...
   time I got through, the line got cut suddenly and I had to call back. This is just not helpful. When I finally got through like after 3 days (yes, 3 days) your agent kept going over a long checklist of trivial things and asking me to verify, after I 
   ...
   Summarize the list of issues is as follows:
   ```
...
2. Ensure that you are using **Greedy** mode and set **Max tokens** to **150**.
...
4. Select the **granite-13b-instruct-v2** model and click **Generate**. You should see the following output.
   ...
   ![json_output2](./images/102/json-output3.png)



5. If we add the cursor to the new line in the input, here is what we get:
   ![json_new_line](./images/102/json_new.png)

   This is the valid JSON we are looking for. Again, the smaller granite-13b-instruct-v2 model seems best for the use case.

6. Now let's try with the **codellama-34b-instruct-hf** model, which is trained for code generation. Select this model and click **Generate**. You should see:

   ![json_output3](./images/102/json-output2.png)



   Here we have a valid JSON, but the indentation has some issues, and there is a lot of white space.

6. The last model to try is the llama-2-70b-chat model. Switch to this model and click 
Generate. You will see this output.

![json_llama](./images/102/json-output_llama.png)

On one hand, the llama-2-70b-chat does generate the proper JSON code. However, as 
you have seen large models tend to be “talkative” and the LLM certainly continues to 
generate – as can be seen by extra text at the bottom. The LLM stopped because it ran 
out of tokens or more text would be generated. 

While this is not a big issue, we will now use the Stop sequences configuration setting to eliminate the extra text starting from I tried using the …
